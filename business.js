...
function upsertCustomer(state, input = {}) {
  ...
  if (!phone && !name) throw new Error("Customer phone or name is required");

  const existing = phone
    ? state.customers.find((item) => normalizePhone(item.phone) === phone)
    : null;

  if (existing) {
    if (name) existing.name = name;
    if (input.source) existing.source = String(input.source);
...
function upsertLeadFromWhatsApp(state, input = {}) {
  const phone = normalizePhone(input.phone);
  if (!phone) throw new Error("WhatsApp sender phone is required");
  ...
  const existing = state.leads.find(
    (item) => normalizePhone(item.phone) === phone && !["completed", "cancelled"].includes(item.stage)
  );

  if (existing) {
    if (input.request) existing.request = String(input.request).trim();
    existing.customerId = customer.id;
    existing.updatedAt = new Date().toISOString();
    return { customer, lead: existing, created: false };
  }
...
function normalizeWhatsAppEvent(payload = {}) {
  const messages = [];
    ...
    for (const change of Array.isArray(entry.changes) ? entry.changes : []) {
      ...
      for (const message of Array.isArray(value.messages) ? value.messages : []) {
        const contact = Array.isArray(value.contacts)
          ? value.contacts.find((item) => normalizePhone(item.wa_id) === normalizePhone(message.from))
          : null;
        messages.push({
          messageId: message.id || null,
          from: normalizePhone(message.from),
          name: String(contact?.profile?.name || "").trim(),
...
function businessSummary(state) {
  ...
  const activeLeads = state.leads.filter((l) => !["completed", "cancelled"].includes(l.stage));
