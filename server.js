...
async function kvGet(key) {
  ...
  const response = await fetchJson(`${base}/get/${encodeURIComponent(key)}`, {
...
function isFinanciallySensitive(text = "") {
  return /دفع|شراء|تحويل|سحب|استلام\s*أموال|استلام\s*اموال|بنك|بطاقة|bank|card|payment|purchase|transfer|withdraw/i.test(text);
}
...
async function verifyConnector(id) {
  try {
    ...
    if (id === "database") {
      ...
      const r = await fetchJson(`https://graph.facebook.com/${WHATSAPP_API_VERSION}/${encodeURIComponent(process.env.WHATSAPP_PHONE_NUMBER_ID)}?fields=id,display_phone_number,verified_name`, {
      ...
      const r = await fetchJson("https://www.googleapis.com/youtube/v3/channels?part=id,snippet&mine=true", {
      ...
      const r = await fetchJson("https://open.tiktokapis.com/v2/user/info/?fields=open_id,display_name", {
      ...
      const r = await fetchJson(`https://${domain}/admin/api/2026-07/shop.json`, {
        headers: { "X-Shopify-Access-Token": process.env.SHOPIFY_ACCESS_TOKEN, Accept: "application/json" }
      });
      ...
      return { id, status: "unknown", verified: false, reason: "Unsupported connector" };
    }
...
function readiness() {
  ...
  for (const [id, connector] of Object.entries(state.connectors)) {
    if (["needs_setup", "configured_unverified", "error"].includes(connector.status) && !["github", "vercel"].includes(id)) {
      blockers.push({
        id,
        status: connector.status,
        priority: id === "database" ? 1 : id === "whatsapp" ? 2 : 3,
        owner: "user",
        action: connector.status === "configured_unverified" ? `تشغيل التحقق الفعلي لـ ${connector.label}` : `إكمال اعتماد ${connector.label}`
      });
    }
  }
  ...
  return {
    ...
    approvalsPending: state.approvals.filter((item) => item.status === "pending").length
  };
}
...
function executionPlan() {
  ...
  return {
    ...
    completed: [
      ...
      "real_connector_verification",
      "autonomous_cycle",
      "business_domain_layer",
      "business_api",
      "whatsapp_webhook_contract",
      "whatsapp_signature_validation"
    ],
...
function systemSelfTest() {
  const checks = [
    ...
    { id: "approval_store", ok: Array.isArray(state.approvals) },
    ...
    { id: "whatsapp_signature_engine", ok: typeof isValidWhatsAppSignature === "function" },
...
async function executeSafeJob(job) {
  if (!job) return { executed: false, reason: "job_not_found" };

  const sensitive = Boolean(job.financial || isFinanciallySensitive(`${job.title} ${job.description}`));
  if (sensitive) {
    job.financial = true;
    if (job.status !== "approved") {
      job.status = "approval_required";
      audit("financial_execution_blocked", { jobId: job.id, reason: "human_approval_required" });
      return { executed: false, reason: "financial_approval_required" };
    }

    job.status = "financial_execution_authorized";
    job.execution = {
      mode: "human_approved",
      result: "authorization_recorded_only",
      externalFinancialExecution: false,
      note: "الموافقة البشرية سُجلت، لكن النظام لا ينفذ حركة مالية فعلية دون موصل مالي صريح وموافقة تشغيلية مناسبة."
    };
    job.executedAt = null;
    audit("financial_action_authorized_not_executed", { jobId: job.id });
    return { executed: false, authorized: true, reason: "financial_execution_not_configured" };
  }

  if (job.status !== "pending") return { executed: false, reason: "not_runnable" };
  job.status = "executed";
...
async function runAutonomousCycle() {
  ...
  const verificationIds = Object.keys(state.connectors).filter(
    (id) => !["github", "vercel"].includes(id) && ["configured_unverified", "error"].includes(state.connectors[id].status)
  );
  const verification = [];
  ...
  for (const job of state.jobs) {
    if (!["pending", "approved"].includes(job.status)) continue;
    const result = await executeSafeJob(job);
    if (result.executed) executed.push(result);
    else skipped.push({ jobId: job.id, reason: result.reason, authorized: Boolean(result.authorized) });
  }
...
app.post("/api/jobs", async (req, res) => {
  ...
  state.jobs.push(job);
  if (financial) createApproval(job);
...
app.post("/api/jobs/:id/approve", async (req, res) => {
  const job = state.jobs.find((x) => x.id === req.params.id);
  ...
  if (!job.financial) {
    ...
    job.status = "approved";
    job.approvedAt = new Date().toISOString();
    const approval = state.approvals.find((item) => item.jobId === job.id && item.status === "pending");
    if (approval) {
      approval.status = "approved";
      approval.approvedAt = job.approvedAt;
    } else {
      const created = createApproval(job);
      created.status = "approved";
      created.approvedAt = job.approvedAt;
    }
  }
...
app.post("/api/jobs/:id/reject", async (req, res) => {
  ...
  if (approval) {
    ...
    approval.rejectedAt = job.rejectedAt;
  }
...
app.post("/api/jobs/:id/execute", async (req, res) => {
  ...
  if (!job) return res.status(404).json({ error: "المهمة غير موجودة" });
  const result = await executeSafeJob(job);
  if (result.reason === "financial_approval_required") {
    await saveState();
    return res.status(403).json({ error: "موافقة المستخدم مطلوبة قبل أي إجراء مالي", status: "approval_required" });
  }
  if (result.reason === "financial_execution_not_configured") {
    await saveState();
    return res.status(202).json({ ok: true, job, status: job.status, note: "تم تسجيل الموافقة البشرية فقط؛ لم يتم تنفيذ أي حركة مالية فعلية." });
  }
  if (!result.executed) return res.status(409).json({ error: "المهمة ليست قابلة للتنفيذ", status: job.status });
  await saveState();
  res.json({ ok: true, job, note: "تم تنفيذ المهمة الداخلية الآمنة فقط؛ لا يوجد إجراء مالي أو خارجي تلقائي هنا." });
});
...
app.get("/api/connectors/status", (req, res) => {
  ...
  res.json({
    ...state.connectors,
    summary: {
      connected: values.filter((x) => x.status === "verified" || x.status === "connected").length,
      deployed: values.filter((x) => x.status === "deployed").length,
      configured: values.filter((x) => x.status === "configured_unverified").length,
      needsSetup: values.filter((x) => x.status === "needs_setup").length,
      errors: values.filter((x) => x.status === "error").length
    },
    persistence: { enabled: persistence.enabled, provider: persistence.enabled ? "KV-compatible REST" : "memory" }
  });
});

app.get("/api/connectors/requirements", (req, res) => res.json({
  github: "متصل عبر GitHub Connector",
  vercel: "النظام منشور على Vercel",
  database: "KV_REST_API_URL + KV_REST_API_TOKEN",
  whatsapp: "WHATSAPP_ACCESS_TOKEN + WHATSAPP_PHONE_NUMBER_ID + WHATSAPP_VERIFY_TOKEN + WHATSAPP_APP_SECRET (موصى به للتحقق من التوقيع)",
  youtube: "YOUTUBE_ACCESS_TOKEN عبر OAuth 2.0",
  tiktok: "TIKTOK_ACCESS_TOKEN عبر OAuth 2.0",
  linkedin: "LINKEDIN_ACCESS_TOKEN عبر OAuth 2.0",
  shopify: "SHOPIFY_ACCESS_TOKEN + SHOPIFY_STORE_DOMAIN",
  financialRule: "لا يتم الدفع أو الشراء أو التحويل أو استلام الأموال دون موافقة المستخدم، ولا توجد حركة مالية فعلية مدمجة في النواة."
}));
...
app.get("/api/whatsapp/webhook", (req, res) => {
  ...
  if (mode === "subscribe" && token === process.env.WHATSAPP_VERIFY_TOKEN) {
    audit("whatsapp_webhook_verified", {});
...
app.post("/api/whatsapp/webhook", async (req, res) => {
  if (!isValidWhatsAppSignature(req)) {
    audit("whatsapp_webhook_signature_rejected", {});
  ...
  audit("whatsapp_webhook_received", {
...
app.get("/api/automation/status", (req, res) => {
  const connectors = Object.entries(state.connectors).map(([id, connector]) => ({
    ...
    externallyExecutable: connector.status === "verified" && connector.automation === "ready"
  }));
  res.json({
    mode: "safe_autonomy",
    financialApprovalRequired: true,
    externalActionsAllowedOnlyForVerifiedConnectors: true,
    financialExternalExecution: false,
    autonomousCycleEndpoint: "/api/automation/cycle",
    connectors,
...
app.get("/api/automation/cycle", async (req, res) => {
  try {
    res.json(await runAutonomousCycle());
  } catch (error) {
    audit("autonomy_cycle_error", { message: error.message });
    await saveState();
    res.status(500).json({ ok: false, error: "autonomy_cycle_failed" });
  }
});
...
loadState().finally(() => {
  audit("system_started", {
    persistence: persistence.enabled,
    autonomousCycle: true,
    businessLayer: true,
    whatsappWebhook: true,
    whatsappSignatureValidation: Boolean(process.env.WHATSAPP_APP_SECRET),
    financialExternalExecution: false
  });
