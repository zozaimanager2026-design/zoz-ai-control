...
const TOOLS = {
  phone:{mode:"mobile_control",purpose:"استلام ومتابعة ومراجعة وتسليم",execution:false},
  github:{mode:"connector",purpose:"source_control_and_delivery",execution:true},
  vercel:{mode:"connector",purpose:"web_deployments",execution:true},
  codex:{mode:"connector_or_executor",purpose:"software_execution",execution:true},
  design_tools:{mode:"external_tool",purpose:"design",execution:true},
  document_tools:{mode:"external_tool",purpose:"documents_pdf",execution:true},
  media_tools:{mode:"external_tool",purpose:"image_video",execution:true},
  automation_tools:{mode:"connector",purpose:"workflows_apis",execution:true},
  data_tools:{mode:"runtime_or_external_tool",purpose:"analysis",execution:true}
};
...
function selectExecutionTools(service,input={}){
 ...
 const usable=t=>TOOLS[t]?.execution===true&&!unavailable.has(t)&&(!hasExplicitAvailability||available.has(t));
 const selected=declared.filter(usable);
 const blockedPrimary=declared.filter(t=>!usable(t));
 const fallback=(service?.alternatives||[]).filter(t=>usable(t)&&!selected.includes(t));
 const selectedTools=[...new Set([...selected,...fallback])];
 const mode=selectedTools.length?((selected.length?"primary":"fallback")):"blocked";
 return {selectedTools,primaryTools:selected,blockedPrimaryTools:blockedPrimary,fallbackTools:fallback,executionMode:mode,availableTools:hasExplicitAvailability?[...available]:null,unavailableTools:[...unavailable]};
}
function buildStages(service){
 const base=["discover_requirements","prepare_assets","execute","quality_check","package_delivery","delivery","follow_up"];
 if(service?.id==="content"||service?.id==="media")return ["discover_requirements","prepare_assets","execute","quality_check","package_delivery","delivery","follow_up"];
 return base;
}
...
function createInitialState(){return {version:3,autonomy:{enabled:true,durable:true,mobileControl:true,financialApprovalRequired:true,lastCycleAt:null,nextCycleAt:null},tasks:[],opportunities:[],skills:SKILLS,tools:TOOLS,experience:[],audit:[]};}
...
function createTask(state,input={}){const plan=buildPlan(input),task={id:id("exec"),title:plan.title,description:plan.description,clientRef:input.clientRef||null,source:input.source||"manual",serviceId:plan.serviceId,status:plan.financial?"approval_required":plan.executionBlocked?"blocked":"queued",stage:plan.stages[0],plan,financial:plan.financial,humanApprovalRequired:plan.humanApprovalRequired,autoExecutable:plan.autonomous,createdAt:now(),updatedAt:now()};state.tasks.push(task);audit(state,"execution_task_created",{taskId:task.id,serviceId:task.serviceId,skills:plan.skills,deliverables:plan.deliverables,tools:plan.tools,executionMode:plan.toolRouting.executionMode,financial:task.financial});return task;}
function advanceSafeTask(state,task){
 ...
 audit(state,"execution_stage_advanced",{taskId:task.id,serviceId:task.serviceId||null,stage:task.stage,executionMode:task.plan.toolRouting.executionMode,selectedTools:task.plan.tools});
 return {taskId:task.id,executed:true,stage:task.stage,status:task.status,executionMode:task.plan.toolRouting.executionMode,selectedTools:task.plan.tools};
}
...
module.exports={SKILLS,TOOLS,isFinanciallySensitive,inferSkills,scoreOpportunity,selectExecutionTools,buildPlan,createInitialState,createTask,advanceSafeTask,runCycle,createStore};
