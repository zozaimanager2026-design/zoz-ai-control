...
function buildData(input = {}) {
  const profile = String(process.env.ZOZ_HF_PROFILE || "qwen-image").toLowerCase();
  ...
  while (Date.now() < deadline) {
    ...
    if (image) {
      return {
        ...
        model: process.env.ZOZ_HF_MODEL || "Qwen/Qwen-Image",
        device: "cuda",
