...
const RENDERERS = Object.freeze({
  ...
  portfolio_case_study: { id: "portfolio_case_study_renderer", section: "portfolio_case_study", capabilities: ["client_portfolio", "brand_case_study", "presentation", "brand_system", "content_system", "qa_package"], queue: "zoz.render.portfolio" },
...
const RENDER_POLICY = Object.freeze({ executionPreference: ["self-hosted-open-source", "free-cloud", "paid-cloud"], paidFallbackAllowed: true, paidFallbackRequiresNeed: true, financialApprovalStillRequired: true });
...
function resolveRenderer({ serviceId, skills = [], deliverables = [] } = {}) {
  ...
  const bySkill = ["portfolio_case_study", "media", "brand_identity", "websites", "automation", "software", "content_writing", "data"];
  ...
  for (const [section, renderer] of Object.entries(RENDERERS)) if (deliverables.some(item => renderer.capabilities.includes(item))) return renderer;
  return null;
}
function buildRendererJob(task) {
  ...
  return { ok: true, rendererId: renderer.id, section: renderer.section, queue: renderer.queue, jobId: task.id, input: { title: task.title, description: task.description, skills: task.plan?.skills || [], deliverables: task.plan?.deliverables || [] }, approval: { financial: Boolean(task.financial), humanApprovalRequired: Boolean(task.humanApprovalRequired) }, renderPolicy: getRenderPolicy(), status: "queued" };
}
