...
function authorized(req) {
  ...
  return req.headers.authorization === `Bearer ${INTERNAL_SECRET}`;
}

function body(req) {
  return new Promise((resolve, reject) => {
    let data = "";
    req.on("data", chunk => { data += chunk; if (data.length > 1024 * 1024) reject(new Error("payload_too_large")); });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/queue") {
      ...
      const result = runtime.queue(task);
      return json(res, result.ok === false ? 400 : 202, result);
    }
    if (req.method === "POST" && req.url === "/improve") {
      const improvement = await body(req);
      return json(res, 201, runtime.recordImprovement(improvement));
    }
    return json(res, 404, { ok: false, error: "not_found" });
  } catch (error) {
    return json(res, 500, { ok: false, error: error.message || "renderer_runtime_error" });
  }
});

server.listen(PORT, HOST, () => {
  process.stdout.write(`ZOZ independent renderer listening on ${HOST}:${PORT}\n`);
});
