...
function inspectLibrary() {
  ...
  return { ok: failures.length === 0, rendererCount: Object.keys(RENDERERS).length, failures, policy: getRenderPolicy() };
}
...
function queue(task) {
  ...
  const queued = { ...job, createdAt: new Date().toISOString() };
...
// Core execution is intentionally deterministic. Provider-specific work can be attached
// through adapters later; the library itself never requires a provider to stay alive.
function execute(job, adapters = {}) {
  ...
  if (job.approval?.financial && job.approval?.humanApprovalRequired !== false) {
    return { ok: false, status: "approval_required", reason: "financial_approval_required" };
  }
  const adapter = adapters[renderer.id];
  const result = typeof adapter === "function"
    ? adapter(job.input)
    : { ok: true, mode: "core-render", rendererId: renderer.id, section: renderer.section, deliverables: job.input?.deliverables || [] };
...
function status() {
  const state = loadState();
  return { ...state, library: inspectLibrary(), selfDevelopment: { ...state.selfDevelopment } };
}

module.exports = { createRuntimeState, inspectLibrary, queue, execute, recordImprovement, status, STATE_FILE, IMPROVEMENTS_FILE };
